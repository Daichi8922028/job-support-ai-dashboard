...
const CompanyAnalysisScreen: React.FC = () => {
  ...
  const handleSendMessage = async (e?: React.FormEvent, messageText?: string) => {
    ...
    setTimeout(() => {
      const mockResponse = `「${companyName}」の「${textToSend}」についてですね。模擬的な回答ですが、${companyName}は${industry}業界で注目されており、特に${textToSend.includes('強み') ? '技術力と市場シェア' : textToSend.includes('弱み') ? '新規事業への展開の遅れ' : '〇〇の分野'}で知られています。より詳しい情報は、企業の公式サイトやIR情報を確認することをお勧めします。`;
  ...
  return (
    <Card title={`企業分析: ${companyName} (${industry})`} className="flex flex-col h-[calc(100vh-8rem)] md:h-[calc(100vh-10rem)]">
      <div className="flex justify-between items-center mb-3 pb-3 border-b">
        <Button onClick={() => setAnalysisStarted(false)} size="sm" variant="secondary">
          別の企業を分析
        </Button>
        <Button onClick={() => alert('リサーチまとめページへ遷移します')} size="sm" variant="primary" leftIcon={<DocumentTextIcon className="w-4 h-4"/>}>
          リサーチまとめを見る
        </Button>
      </div>
